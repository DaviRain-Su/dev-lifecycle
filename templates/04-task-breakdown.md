...
### 4.2.1 任务性质：探索型 vs 承诺型

> 不是所有任务都需要同样的流程重量。区分任务性质，让速度用在对的地方。

| | 探索型 (Explore) | 承诺型 (Commit) |
|---|-----------------|----------------|
| **目标** | 验证方向、快速学习、减少不确定性 | 交付生产代码，长期维护 |
| **代码去向** | 可丢弃，**不合入 main** | 合入 main，长期存在 |
| **流程要求** | 可跳过 Phase 3（技术规格）和 Phase 5（测试规格） | 必须走完整 7 个阶段 |
| **代理使用** | 全速推进，允许快速迭代 | 遵守熵检查和漂移监控 |
| **分支规范** | `explore/xxx` 或 `prototype/xxx` | `feature/xxx` |
| **典型场景** | 产品方向探索、技术选型验证、原型演示、repro case | 核心功能、数据模型、API、基础设施 |

**⚠️ 关键规则**:
- 探索型任务的产出物必须标记为 `prototype-only, do not merge to main`
- 如果探索型任务的成果需要进入生产，必须**重新走承诺型流程**（从 Phase 3 开始）
- 探索型任务不计入项目完成度——它们是学习工具，不是交付物

### 任务列表

| # | 任务名称 | 描述 | 依赖 | 类型 | 性质 | 预估时间 | 优先级 | Done 定义 |
|---|---------|------|------|------|------|---------|--------|----------|
| T1 | | | 无 | 串行 | 承诺 | h | P0 | 当 X 时，Y 成立 |
| T2 | | | T1 | 并行 | 承诺 | h | P0 | |
| T3 | | | T1 | 并行 | 探索 | h | P1 | |
